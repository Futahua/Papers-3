 ...
 *
 * This mounts ONLY when Papers is launched with PAPERS_ENABLE_FIXTURES=1. It is
 * never part of the production shell; it exists so the pinned engineering
 * fixtures (kill test, repository workflow, Logseq) keep exercising the Canvas
 * runtime, agent runs and capability broker end to end.
 */
export function FixtureApp(): React.JSX.Element {
  ...
  // Fixture-only: auto-restore the last active Backpack (and its program) on
  // launch, so the pinned relaunch/restore regressions keep exercising the
