...
export interface PapersWindowFactoryOptions {
  ...
  onProjectViewLoaded?: (view: WebContentsView, windowId: number, surfaceId: string, projectId: string) => Promise<void> | void;
  rendererUrl?: string;
  rendererFile: string;
}
...
export function createPapersWindow(options: PapersWindowFactoryOptions): PapersWindowInstance {
  ...
  const projectSurfaces = new BackpackProjectSurfaceCollection(
    ...
    undefined,
    options.onProjectViewLoaded
      ? (view, surfaceId, projectId) => options.onProjectViewLoaded!(view, window.id, surfaceId, projectId)
      : undefined,
  );
