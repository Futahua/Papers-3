...
const cancel=()=>signal('cancel');document.querySelector('.close').onclick=cancel;search.oninput=render;currentFilter.onchange=render;availableFilter.onchange=render;document.addEventListener('keydown',e=>{if(e.key==='Escape'){e.preventDefault();cancel()}else if(e.key==='ArrowDown'){e.preventDefault();list.querySelector('.row')?.focus()}});render();search.focus();
</script>`;
      return new Promise<{ action: 'select' | 'close' | 'cancel'; candidateId: string | null }>((resolve) => {
