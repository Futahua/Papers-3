...
| Local Backpack project bindings | `Papers/Data/PapersData/backpack-projects.json` | Papers host seam / this machine | **Machine-local; never infer another machine's binding** | No, but contains absolute local paths | Agents write deliberately; Papers reads | Recreate from the independently preserved project; never encode the binding in a Backpack record |
| “As you Go” project and prepared actions | `Papers/Backpack projects/As you Go` | “As you Go,” not the Papers release | **Local to this machine; do not sync, publish or migrate by inference** | No expected secrets; action and state records contain absolute local paths, and custom icons may be embedded image data | One local project; Papers reads the project and atomically mediates state writes | Project source has its own Git history; live `state.json` is intentionally ignored by Git and preserved in place |
| “As you Go” explorer state | `Papers/Backpack projects/As you Go/state.json` | The creator's local “As you Go” experience | **Machine-local**; no Papers release or other-machine update | No expected secrets; contains shortcut targets, optional descriptions/icons, nesting, manual order, Bin recovery records and icon-size preference | One local project instance; queued atomic writes prevent overlapping saves | Delete first moves items into the in-project Bin; Restore returns them to their recorded location; permanent deletion requires a separate confirmed action inside the Bin |
