...
| “As you Go” project and prepared actions | `Papers/Backpack projects/As you Go` | “As you Go,” not the Papers release | **Local to this machine; do not sync, publish or migrate by inference** | No, but `actions.json` contains absolute local paths | One local project; Papers reads on entry/action | Its own local Git history; ordinary changes stay outside `App` |
