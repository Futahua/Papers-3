...
The OpenCode worker lane runs `opencode run --pure --dir <worktree> -m opencode/big-pickle --dangerously-skip-permissions` with a runtime `OPENCODE_CONFIG_CONTENT` policy. OpenCode documents that auto-approval does not override explicit denies; the policy denies external-directory access, network tools, subagents, pushes, history rewrites, and destructive shell commands. The exact command and policy are shown in the Papers invocation preview and remain gated by Hermes' approval. The disposable worktree protects the base repository, but this is explicitly treated as defense in depth rather than an OS sandbox. Codex uses non-interactive `codex exec --sandbox workspace-write --ask-for-approval never` with the native Windows `unelevated` backend, the documented fallback when elevated sandbox setup fails; there is no sandbox bypass. Worker model pinned to `opencode/big-pickle` for provider diversity versus Codex's ChatGPT account (§16.3).
