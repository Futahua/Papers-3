...
async function boundedCall(promise, deadline, signal) {
  if (signal?.aborted) throw abortError();
  const remaining = deadline - Date.now(); if (remaining <= 0) throw new Error('incident transcript setup timed out');
  return new Promise((resolveResult, rejectResult) => {
    let finished = false; const timer = setTimeout(() => finish(undefined, new Error('incident transcript setup timed out')), remaining);
    const onAbort = () => finish(undefined, abortError());
    const finish = (value, error) => { if (finished) return; finished = true; clearTimeout(timer); signal?.removeEventListener('abort', onAbort); error ? rejectResult(error) : resolveResult(value); };
    signal?.addEventListener('abort', onAbort, { once: true });
    Promise.resolve(promise).then((value) => finish(value), (error) => finish(undefined, error));
  });
}
...
export async function collectIncidentTranscript(connection, target, { durationMs = 60_000, maxRecords = MAX_RECORDS, maxBytes = MAX_BYTES, signal, deadline: suppliedDeadline } = {}) {
  ...
  try {
    ...
    const subscription = await boundedCall(connection.call('events.subscribe', { events: ['visual.lifecycle', 'visual.diagnostic'], visualTarget: target }), deadline, signal);
    if (!subscription?.ok) throw new Error(subscription?.error ?? 'visual event subscription failed');
    const [initial, windowInitial] = await Promise.all([
      boundedCall(connection.call('inspect.visual.diagnostics', target), deadline, signal),
      boundedCall(connection.call('inspect.visual.diagnostics', { windowId: target.windowId }), deadline, signal),
    ]);
    if (!initial?.ok || !windowInitial?.ok) throw new Error(initial?.error ?? windowInitial?.error ?? 'visual diagnostics inspection failed');
