...
export async function launchPapers(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      // Publishes the launcher's own open path over IPC so a test can press the
      // chord without synthesising a system-wide accelerator. Never set in a
      // normal run.
      ...(options.testInvokeChannel ? { PAPERS_TEST_INVOKE_CHANNEL: '1' } : {}),
