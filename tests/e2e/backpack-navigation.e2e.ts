...
it('hover picker and main-screen clicks reuse tabs, middle-click adds, and split borders resize', async () => {
  ...
  try {
    ...
    await page.getByRole('tab', { name: 'Beta' }).click({ button: 'middle' });
    await tabCount(1);
    expect(await page.getByRole('tab', { name: 'Gamma' }).count()).toBe(1);
    await page.locator('.titlebar-left > button').click();
    await mainEnter('Beta').click({ button: 'middle' });
    await tabCount(2);
    const emptyHeader = page.locator('.dv-void-container').first();
    const emptyBox = await emptyHeader.boundingBox();
    expect(emptyBox).not.toBeNull();
    await page.mouse.move(emptyBox!.x + emptyBox!.width / 2, emptyBox!.y + emptyBox!.height / 2);
    await page.mouse.down();
    await page.mouse.move(emptyBox!.x + emptyBox!.width / 2, emptyBox!.y + 160, { steps: 8 });
    await page.mouse.up();
